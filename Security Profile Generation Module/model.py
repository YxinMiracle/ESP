...
class TenorNetworkModule(torch.nn.Module):
    ...
    def __init__(self, hidden_dim, tensor_neurons):
        """
    ...
    def forward(self, embedding_1, embedding_2, embedding_3):
        """
        ...
        """
        bsz = embedding_1.shape[0]
        scoring = torch.matmul(embedding_1, self.weight_matrix.view(self.hidden_dim, -1))
        scoring = scoring.view(bsz, self.hidden_dim, -1).permute([0, 2, 1])
        scoring = torch.matmul(scoring, embedding_2.view(bsz, self.hidden_dim, 1)).view(bsz, -1)
        combined_representation = torch.cat((scoring, embedding_3), 1)
        block_scoring = torch.t(
            torch.mm(self.weight_matrix_block, torch.t(combined_representation))
        )
        scores = torch.nn.functional.relu(embedding_1 + block_scoring + self.bias.view(-1))
        return scores
...
#
#         self.fc_out = torch.nn.Linear(hidden_dim, hidden_dim)
#
#     def forward(self, current_wordseq_feat, total_word_embedding):
#         bsz, seq_len, _ = total_word_embedding.shape
#
#         # Split the embedding into `num_heads` heads
#         def split_heads(x):
#             return x.view(bsz, -1, self.num_heads, self.head_dim).permute(0, 2, 1, 3)
#
#         query = split_heads(self.query(current_wordseq_feat))
#         key = split_heads(self.key(total_word_embedding))
#         value = split_heads(self.value(total_word_embedding))
#
...
#
#         # Concatenate heads
#         out = out.permute(0, 2, 1, 3).reshape(bsz, 1, -1)
#
#         # Pass through final linear layer
#         out = self.fc_out(out)
#         return out  # (bsz, 1, hidden_dim)
...
class AttAttYxinNerModel_POS(nn.Module):
    def __init__(self):
        ...
        self.model = AutoModelWithLMHead.from_pretrained("bert-base-cased", config=config)
    ...
    def forward(self, X, y, pos_data):
        ...
        outputs = self.model(X,
                             truncation=True,
                             max_length=510)  # a tuple ((bsz,seq_len,hidden_dim), (bsz, hidden_dim))
