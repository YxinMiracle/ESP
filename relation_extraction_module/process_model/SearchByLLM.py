...
class LLM_RE:
    GET_REASON_TEMPLATE = """
    You are an expert in the field of cybersecurity threat intelligence. Next, I will have some preproces sing tasks related to extracting relationships from threat intelligence texts. The specific task is as follows: I will provide you with a JSON string, formatted as:
    ...
    The last part is "Test Input", for which I will input a JSON stQwen2-72B-Instructring, {"sent": "specific sentence", "head_entity_name": "head entity name", "head_entity_type": "head entity type", "tail_entity_name": "tail entity name", "tail_entity_type": "tail entity type"}. This JSON string is similar to the ones above but lacks the reason and relation. You need to determine the relationship and respond with a JSON string, such as {"relation_type":"uses"} or {"relation_type":"NULL"}...etc., to indicate what the relationship between the two entities in the input sentence should be. Note, please do not output any extra content.
    """
    ...
    def __init__(self, params, isApi=False):
        self.params = params
        ...
        if not isApi: self.llm_model, self.llm_tokenizer = self._init_llm_model()
    ...
    def final_ans(self, origin_sent: ReSentBaseData, demonstration_answer_list: List[dict]):
        ...
        relation_list = self.entity_relation_rule_dict[origin_sent.head_entity.entity_type][
                            origin_sent.tail_entity.entity_type] + ["NULL"]  # type: List[str]
    ...
    def do_llm_search(self, origin_sent: ReSentBaseData, similar_sent_list: List[IdentifiedReSentBaseData]):
        ...
        answer_dict = self.final_ans(origin_sent, demonstration_answer_list)
        return self.get_llm_answer(prompt=self.GET_RE_ANSWER_TEMPLATE, question=json.dumps(answer_dict))


    def do_llm_api_search(self, origin_sent: ReSentBaseData, similar_sent_list: List[IdentifiedReSentBaseData], api_model_name):
        demonstration_answer_list = self.get_gold_label_induced(similar_sent_list)
        answer_dict = self.final_ans(origin_sent, demonstration_answer_list)
        return self.get_llm_answer(prompt=self.GET_RE_ANSWER_TEMPLATE, question=json.dumps(answer_dict))
